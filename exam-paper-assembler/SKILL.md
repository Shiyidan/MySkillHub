...
10. 最终项目 JSON 的根字段必须为 `metadata`、`sections`；真题诊断卷使用 `examType=ESAT`、`paperType=realPaper`、`assemblyType=legacy_equivalent`、`deliveryMode=section_sequence`。`sections[0..2]` 是三个科目段，`sectionType=subject`。
11. `out_of_scope`、`partially_in_scope`、`unknown` 默认不得进入诊断卷。
12. 同一道来源题进入不同组合卷时必须保留相同 `sections[].questions[].code`；每个科目段内题号从 1 重新连续编号。题目的学科、章节和知识点统一放入 `classification`，来源放入 `source`，不得再输出旧的根级题目分类字段或 `is_ai_generated`。
...
- 每套 JSON 的 `sections` 必须包含三个科目段；每段只包含 `code`、`sectionType`、`order`、`questions`，顺序与组合科目顺序一致。时间、休息和跳转由项目端配置。
- 每套 JSON 的 `metadata.remarks` 必须逐模块写明实际/目标题量、覆盖警告和诊断可信度，供管理员导入前核对。
- 每套卷按 Mathematics 1、进一步模块一、进一步模块二排列模块；模块内题目从 1 连续编号。
- 每题使用项目标准的 `title`、`contentBlocks`、数组 `answer`、`classification`、`source` 和 `learningAnalysis`。
