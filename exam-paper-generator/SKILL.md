...
本 Skill 只从已通过校验的 canonical `parsed_exam` 生成新题，并输出内部 canonical `generated_exam`。生成题默认不进入由 ENGAA+NSAA 真题构成的六套 legacy 诊断卷。质量来自逐题蓝图、唯一母题、推理结构重构、独立求解和新颖性复核；最终质量门只负责兜底拦截。所有面向用户的说明、日志、错误和解析使用中文；新题题干、选项和作答内容保持英文。
...
8. 输出草稿前阅读 `references/output-contract.md` 和 `references/quality-rules.md`；最终输出必须通过共享契约、逐题事务和新颖性校验。
9. 只执行生成流程，不承担证据提取、答案配对或原卷解析。
...
约束文件至少包含正整数 `question_count`，可按需指定考试、模块、题型、难度分布、考纲标签和排除条件。`finalize` 会重新校验输入、题量、唯一母题引用、题目指纹和重复情况，但不会替生成草稿补写质量字段。
